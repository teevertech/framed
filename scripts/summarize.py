...
def _make_env(config: TrainConfig, seed: int) -> PanelEnv:
    """Create a PanelEnv from a single panel drawn at *seed*.

    Uses ``config.make_panel_generator`` so the panel topology matches
    the training distribution exactly — same wall-length pool, opening
    types, and widths.
    """
    gen = config.make_panel_generator(seed=seed)
    panel = gen()
    return PanelEnv(
        panel,
